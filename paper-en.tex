...
tweets~\cite{Arolfo2020}. Recent user studies of health-related posts also show
that source characteristics, evidence type, and prior-belief consistency shape
credibility judgments, with prior beliefs and source expertise being especially
influential~\cite{Kuutila2024}.
...
professional-looking link. This decomposition is also consistent with empirical
work on health-related social media posts: Kuutila et al. show that readers'
credibility judgments are shaped by source characteristics, evidence type, and
prior-belief consistency, and that prior beliefs and source expertise can be
especially influential~\cite{Kuutila2024}. In the proposed model, expertise
operationalizes source characteristics, verifiability captures the availability
and quality of supporting evidence, and reputation is kept separate so that
audience signals do not stand in for domain competence or evidence.
...
textual reuse. This also motivates using small language models (SLMs) for
image-based clip processing, such as screenshots or rendered posts, as access to
social media APIs is increasingly restricted and heterogeneous across platforms.
These SLMs should run at the edge, without sending clips to the cloud, to preserve
privacy; the credibility quality model should be integrated into the SLM pipeline
so that extracted visual and textual evidence is mapped directly to quality
