...
Normalization can be implemented automatically when the platform exposes the
required fields through an API or export, and semi-automatically when access is
limited. In both cases, platform-specific ingestion maps the available fields to
the common JSON structure; missing fields remain explicit null or unknown values
rather than being silently inferred. This preserves comparability across
platforms while making data availability visible to the quality modules.
...
The mapping from features to quality values may be rule-based, model-based, or
configured by a domain expert. Thus, the processing flow is not tied to a single
automatic classifier: it provides a modular way to combine automated extraction
with expert-defined quality criteria.
...
Equivalence search is applied to a candidate set, not to all possible social
media clips. Candidate sets can be built from explicit links, shared URLs,
hashtags, time windows, user mentions, platform search results, or previously
collected clips in a case study. The current proof of concept does not use social
network graph distance to rank candidates; graph-based neighborhood expansion and
relationship-distance weighting are left as future extensions.
...
Equivalence and provenance were evaluated over this collected set, rather than
through an exhaustive search over all social media content.
