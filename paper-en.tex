...
compared against a medical-domain expert's judgment, matches that judgment for
nine of eleven selected clips and illustrates how provenance can change the
...
Cross-platform sharing adds a provenance challenge: content can be shared through
platform mechanisms or manually copied into another platform. In the manual case,
source metadata may be lost, the content may be modified, and attribution becomes
harder. This makes the relevant social-network aspect a problem of interactions,
derivations, and platform-aware provenance rather than only a classification
problem over isolated posts.

\begin{table*}[t]
  \caption{Remaining gaps in representative related work.}
  \label{tab:related-gap}
  \small
  \begin{tabular}{@{}p{.20\textwidth}p{.24\textwidth}p{.27\textwidth}p{.22\textwidth}@{}}
    \toprule
    Related strand & Representative work & Covered aspects & Remaining gap addressed here\\
    \midrule
    Perceived credibility
      & Fogg and Tseng~\cite{FoggTseng1999}; Kuutila et al.~\cite{Kuutila2024}
      & Credibility facets, user perception, author expertise, and evidence judgment
      & Credibility is not operationalized as auditable data-quality metrics\\
    Platform-specific credibility
      & Castillo et al.~\cite{Castillo2011}
      & Message, user, topic, and propagation features for Twitter/X credibility
      & Focuses on estimation rather than a reusable quality model across platforms and domains\\
    Social media data quality
      & Arolfo et al.~\cite{Arolfo2020}
      & Adaptation of data-quality dimensions to social media streams
      & Credibility is not modeled as a quality cluster combining trustworthiness and provenance\\
    Provenance models
      & Simmhan et al.~\cite{Simmhan2005}; Taxidou et al.~\cite{Taxidou2015}
      & Origin, derivation, diffusion, agents, messages, influence, and interactions
      & Domain-specific credibility metrics are not integrated into provenance paths\\
    \bottomrule
  \end{tabular}
\end{table*}

Table~\ref{tab:related-gap} highlights remaining gaps in representative work
most directly connected to the design requirements of the model; a broader
review is available in the master's thesis~\cite{GarciaParra2024Thesis}. The gap
addressed in this paper is not the absence of credibility classifiers or
...
be modular, so new feature modules, metrics, and provenance search strategies
...
derivation, and propagation path of the clip. This two-dimensional organization
does not exclude reputation; rather, it places reputation inside
trustworthiness, together with verifiability and expertise, because reputation is
one signal used to estimate whether the source associated with a clip should be
trusted.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth,height=.42\textheight,keepaspectratio]{figures/credibility-cluster-tree}
...
credibility~\cite{FoggTseng1999}. These facets describe how credibility is
perceived, whereas the quality factors below them indicate which parts of that
perception the model makes explicit and measurable. Provenance is treated
separately because it captures the clip lifecycle rather than a single perceptual
cue. Presumed
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    Trustworthiness & Reputation & Engagement, confidence\\
     ...
     & Expertise & Certification, influence\\
    \midrule
    Provenance & Attribution & Attribution metadata\\
     & Process & Edit count, edit history\\
     & Path & Path stability\\
    \bottomrule
  \end{tabular}
\end{table}

\autoref{tab:model} summarizes the core dimensions, factors, and example metrics
used by the model. Let $T$ denote trustworthiness and $P$ denote provenance, both
in $[0,1]$. The
...
Normalization maps the attributes of the original clip from each platform to the
attributes of the normalized clip. In the reference implementation, this step is
performed by a Cloud Function, a serverless Google Cloud component used in the
proof-of-concept deployment, which processes each input clip and stores the
resulting normalized JSON object for the following stages.
...
using a formula, a rule, or a model. This separation lets a new feature module
be added without changing the semantics of the quality model.
Feature orchestration depends on the clip type, the fields available in the
normalized clip, the flow configuration, and resource priorities. The data-quality
expert and the domain expert determine which extracted or inferred features are
relevant for the quality metrics of a given use case.
...
The equivalent-clip search module receives a normalized clip and searches one or
more social platforms, using social-network APIs or search engines. In the
reference implementation, explicit interactions in X are obtained through the X
API, while other explicit and implicit cases are searched with Google Custom
Search over Facebook, Instagram, X, TikTok, and Reddit. Returned snippets are
compared with the original clip text using TF-IDF cosine similarity.
...
The three trustworthiness submetrics were computed as normalized values in
$[0,1]$. Access to source data was set to the medical-verifiability feature
extracted for the clip, or 0 when unavailable. Certification combined formal
medical education and declared medical profession; confidence combined medical
influence and audience size:
\begin{align*}
  Certification &= 0.6\,EQ + 0.4\,PQ,\\
  Confidence &= 0.7\,I + 0.3\,\frac{\log(A+1)}{\log(A_{\max}+1)}.
\end{align*}
Here $EQ$ is education quality, $PQ$ is profession quality, $I$ is medical
influence, $A$ is audience size, and $A_{\max}=100{,}000$ is the reference
maximum audience size used for normalization in the proof of concept. Missing
feature values were set to 0. The medical-influence value is generated from
profile-level recommendations or textual comments associated with the author's
medical knowledge and experience in the statins and cholesterol domain. The
aggregation weights for trustworthiness and
credibility were assigned with the medical-domain expert for this case study.
Access to source data and certification received the highest weights because
medical credibility depends strongly on verifiable evidence and professional
credentials. Confidence received a lower aggregation weight to prevent popularity
or audience size from dominating the score in a sensitive health domain. The
credibility weights also reflect the expert criterion that, when provenance is
available, stable propagation through trustworthy accounts should increase the
credibility of the original content.

In this proof of concept, the feature modules focused on medical profile evidence. They
used Gemini 1.5 Flash with task-specific prompts to extract medical profession,
medical education, medical verifiability, and medical influence from public
profile information and available textual evidence. This is an implementation
choice of the proof of concept; the framework only requires feature modules that
produce normalized values for the quality metrics.
...
\begin{table}[t]
  ...
  \begin{tabular}{ccccc}
    \toprule
    Clip & Expert & Expert score & Model & Cred.\\
    \midrule
...
The model agreed with the expert-assigned credibility level in nine of the
eleven clips (\autoref{tab:results}). In the table, \emph{Expert score} is the
numeric score assigned by the domain expert, while \emph{Cred.} is the final
credibility score computed by the model. The two differences corresponded to
clips for which provenance increased the model score: both had stable paths and
were shared by users with relatively high trustworthiness. The expert did not
have access to all profiles involved in the cross-platform propagation path,
which explains why the expert score was closer to trustworthiness alone than to
the final provenance-aware score.

Three qualitative examples illustrate this comparison. The expert assigned Clip~1
a medium score ($0.60$). In the model, the clip was associated with a cardiology
profile with high certification and audience confidence, but limited explicit
access to source data. Its trustworthiness score was therefore medium ($0.62$).
However, the clip was propagated through a stable path with high Trustworthiness
Path Stability ($0.92$), which raised its final model credibility to $0.80$.
The expert also assigned Clip~5 a medium score ($0.60$). In the model, stronger
source and certification signals yielded medium trustworthiness ($0.69$), and a
stable provenance path ($0.83$) raised the final model score to $0.78$. In both
cases, the qualitative explanation is not that the original clip was
automatically high credibility, but that provenance added evidence unavailable in
the expert's isolated view. Conversely, the expert assigned Clip~7 a low score
($0.00$). The model also kept this clip at low credibility ($0.18$), because it
came from a self-described naturopath profile, with weak certification and
limited verifiable source evidence, despite having some audience-related signals.
...
Future work should expand the evaluation to larger datasets, additional social
