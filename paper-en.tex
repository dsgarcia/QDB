...
This paper focuses on measuring credibility in social media platforms from a data-quality perspective. Its main contribution is a provenance-aware data-quality model for
...
tweets~\cite{Arolfo2020}. In the health-oriented analysis that motivates this
model, Kuutila et al. are relevant because they show that readers use author
biographies and perceived domain expertise as credibility cues, and that
consistency with prior beliefs can affect how evidence is judged~\cite{Kuutila2024}.
...
used by different roles: final users who consume credibility indicators, data
...
professional-looking link. This is the point at which Kuutila et al. support the
model design: if readers infer credibility from a profile
biography or from an apparent professional role, then expertise must be measured
as a distinct quality factor rather than absorbed into reputation or popularity;
and if prior-belief consistency can lead readers to discount evidence quality,
then verifiability must remain visible as a separate factor~\cite{Kuutila2024}.

These facets are useful, but they are also failure modes. In social media,
...
In health-related social media, this decomposition is useful because different
weak signals should not be conflated. A popular account may have high reputation
but no medical expertise. A medical specialist may have expertise but provide no
links to source data. A clip may have acceptable trustworthiness in isolation but
become more credible if its provenance shows stable propagation through trusted
medical profiles. The model keeps these cases distinguishable.

This structure is also what makes the model reusable across domains. The abstract
dimensions remain stable, but the metrics and weights are supplied by domain
experts. In medicine, certification and access to source data can dominate the
score. In emergency response, freshness, geolocation, and corroboration across
independent sources may be more important. In political communication, attribution,
bot-like propagation, and source history may receive higher weights. The model is
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth,height=.35\textheight,keepaspectratio]{figures/processing-flow-en}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth,height=.30\textheight,keepaspectratio]{figures/provenance-framework-en}
...
The implemented feature modules focused on medical profile evidence. They
...
to a data-quality workflow. This has two advantages. First, it makes the
...
is a generic proposal that can be instantiated in different domains. Medical
...
for text-only equivalence, but future versions should evaluate whether small
language models (SLMs) can replace or complement it for semantic clip equivalence,
especially when content is paraphrased, translated, rendered as an image, or moved
across platforms. This also motivates using SLMs for image-based clip processing,
such as screenshots or rendered posts, as access to social media APIs is
...
and metrics.
The provenance component should also be extended with explicit
...
A detailed version of the model and its design rationale can be found in the
master's thesis~\cite{GarciaParra2024Thesis}; the code repository is available at
