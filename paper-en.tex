...
In this paper, these concepts are organized hierarchically: reputation is modeled
as one factor that contributes to trustworthiness, while provenance is modeled as
a complementary top-level quality dimension.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}p{.20\textwidth}p{.36\textwidth}p{.37\textwidth}@{}}
    ...
    Social media provenance & Origin, diffusion, interactions, and cross-platform sharing & Quality metrics are not integrated into platform-aware provenance paths\\
    \bottomrule
  \end{tabular}
\end{table*}
...
one signal used to estimate whether a clip or its source should be trusted.
...
performed by a Cloud Function that processes each input clip and stores the
